...
function array_contains($keys, $array) {
	foreach ($keys as $key) {
		if (!isset($array[$key])) {
			return false;
		}
...
function array_missing_key($keys, $array) {
	foreach ($keys as $key) {
		if (!isset($array[$key])) {
			return $key;
		}
...
function curl_process_output($o) {
	$output = array();
...
function object_order($aObj, $property) {
	$cursor = 1;
	$comparison = 1;
...
function arrayarray_order($aObj, $property, $dec=false) {
	$comparisonKey = array();
	$data = array();
	$keyNum = true;
...
function is_interface_of($obj, $interface) {
	$r = new ReflectionClass($obj);
	return $r->implementsInterface($interface);
}

function get_full_interface($iface) {
