...
class WebDriverBase {
	 ...
	 * 
	 * @param Curl_session $session  The driver session
	 * @param array        $response The response from driver
	 * 
	 ...
	 * 
	 * @param string $json_response Response from server
	 * 
	...
	protected function handleResponse($json_response) {
		...
		7 	NoSuchElement 	An element could not be located on
		...
		9 	UnknownCommand 	The requested resource could not be found, or a
							request was received using an HTTP method that is
							not supported by the mapped resource.
		10 	StaleElementReference 	An element command failed because the referenced
							element is no longer attached to the DOM.
		11 	ElementNotVisible 	An element command could not be completed because the
		...
		17 	JavaScriptError 	An error occurred while executing
								user supplied JavaScript.
		19 	XPathLookupError 	An error occurred while searching
								for an element by XPath.
		23 	NoSuchWindow 	A request to switch to a different window
							could not be satisfied because the window
							could not be found.
		24 	InvalidCookieDomain 	An illegal attempt was made to set a
									cookie under a different domain than
									the current page.
		25 	UnableToSetCookie 	A request to set a cookie's value could
								not be satisfied.
	 ...
	 * 
	 * @param string $locatorStrategy A constant from LocatorStrategy
	 * @param string $value	          The search string used for location
	 * 
	 * @see LocatorStrategy
